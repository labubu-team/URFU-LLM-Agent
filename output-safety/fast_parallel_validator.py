...
class FastRegexChecker:
    ...
    def _compile_pii_patterns(self) -> Dict[str, re.Pattern]:
        ...
        patterns = {
            ...
            'email': re.compile(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.\u0430-\u044f\u0410-\u042f-]+\.[A-Za-z\u0430-\u044f\u0410-\u042f]{2,}\b', re.IGNORECASE),
            
            # Улучшенные российские номера телефонов
            'phone_ru_mobile': re.compile(r'\b(?:\+?7|8)[-.\s]?\(?(?:9\d{2}|3\d{2}|4\d{2}|5\d{2})\)?[-.\s]?\d{3}[-.\s]?\d{2}[-.\s]?\d{2}\b', re.IGNORECASE),
            'phone_ru_city': re.compile(r'\b(?:\+?7|8)[-.\s]?\(?(?:3\d{2}|4\d{2}|8\d{2})\)?[-.\s]?\d{2}[-.\s]?\d{2}[-.\s]?\d{2}\b', re.IGNORECASE),
            'phone_intl': re.compile(r'\b\+(?:1|33|44|49|81|86|91)\d{8,14}\b', re.IGNORECASE),
            
            # Улучшенные банковские карты (с базовой валидацией длины)
            'credit_card_visa': re.compile(r'\b4\d{3}[-.\s]?\d{4}[-.\s]?\d{4}[-.\s]?\d{4}\b', re.IGNORECASE),
            'credit_card_mastercard': re.compile(r'\b5[1-5]\d{2}[-.\s]?\d{4}[-.\s]?\d{4}[-.\s]?\d{4}\b', re.IGNORECASE),
            'credit_card_mir': re.compile(r'\b2[2-4]\d{2}[-.\s]?\d{4}[-.\s]?\d{4}[-.\s]?\d{4}\b', re.IGNORECASE),
            'credit_card_generic': re.compile(r'\b\d{4}[-.\s]?\d{4}[-.\s]?\d{4}[-.\s]?\d{4}\b', re.IGNORECASE),
            
            # Российские документы
            'snils': re.compile(r'\b\d{3}[-.\s]?\d{3}[-.\s]?\d{3}[-.\s]?\d{2}\b', re.IGNORECASE),
            ...
            # Технические данные
            'ip_address': re.compile(r'\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b', re.IGNORECASE),
            'ipv6': re.compile(r'\b(?:[0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}\b', re.IGNORECASE),
            'mac_address': re.compile(r'\b(?:[0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}\b', re.IGNORECASE),
            
            # Криптовалюты
            'bitcoin': re.compile(r'\b[13][a-km-zA-HJ-NP-Z1-9]{25,34}\b', re.IGNORECASE),
    ...
    def _compile_sensitive_patterns(self) -> Dict[str, re.Pattern]:
        ...
        patterns = {
            # API ключи и токены
            'api_key_general': re.compile(r'\b(?:api[_-]?key|token|secret|password|pwd|bearer|access[_-]token)\s*[:=]\s*["\']?[\w\-]{8,}["\']?', re.IGNORECASE),
            'openai_key': re.compile(r'\bsk-[A-Za-z0-9]{48,}\b', re.IGNORECASE),
            'github_token': re.compile(r'\b(?:ghp_|gho_|ghu_|ghs_)[A-Za-z0-9]{36}\b', re.IGNORECASE),
            'github_classic': re.compile(r'\b[a-f0-9]{40}\b', re.IGNORECASE),  # Classic GitHub tokens
            'aws_key': re.compile(r'\bAKIA[A-Z0-9]{16}\b', re.IGNORECASE),
            'aws_secret': re.compile(r'\b[A-Za-z0-9+/]{40}\b', re.IGNORECASE),
            
            # Российские сервисы
            'yandex_token': re.compile(r'\b(?:AQVN|AQAf)[A-Za-z0-9_-]{36,}\b', re.IGNORECASE),
            'vk_token': re.compile(r'\b[a-f0-9]{85}\b', re.IGNORECASE),
            'telegram_bot_token': re.compile(r'\b\d{8,10}:[A-Za-z0-9_-]{35}\b', re.IGNORECASE),
            'mailru_token': re.compile(r'\b[A-Za-z0-9]{32,}\b', re.IGNORECASE),
            
            # Медицинские данные (расширенный список)
            'medical_ru': re.compile(r'\b(?:диагноз|болезнь|заболевание|лечение|медицина|врач|доктор|пациент|больной|анализ|результат|рецепт|препарат|лекарство|симптом|синдром)\b', re.IGNORECASE),
            'medical_en': re.compile(r'\b(?:diagnosis|disease|illness|treatment|medical|doctor|physician|patient|test\s+result|prescription|medication|drug|symptom|syndrome)\b', re.IGNORECASE),
            
            # Финансовые данные (расширенный список)
            'financial_ru': re.compile(r'\b(?:зарплата|доход|оклад|долг|кредит|займ|банк|счет|вклад|депозит|налог|пенсия|пособие|льгота|страховка)\b', re.IGNORECASE),
            'financial_en': re.compile(r'\b(?:salary|income|wage|debt|credit|loan|bank|account|deposit|tax|pension|benefit|insurance)\b', re.IGNORECASE),
            
            # Банковские данные
            'swift_iban': re.compile(r'\b(?:SWIFT|BIC|IBAN)\s*:?\s*[A-Z0-9]{8,34}\b', re.IGNORECASE),
            'bik_ru': re.compile(r'\b(?:БИК|BIK)\s*:?\s*\d{9}\b', re.IGNORECASE),
            'swift_code': re.compile(r'\b[A-Z]{6}[A-Z0-9]{2}(?:[A-Z0-9]{3})?\b', re.IGNORECASE),
            
            # Пароли и секреты
            'password_patterns': re.compile(r'\b(?:password|passwd|pwd|secret|pass)\s*[:=]\s*["\']?[^\s"\']{6,}["\']?', re.IGNORECASE),
            'jwt_token': re.compile(r'\beyJ[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]*\b', re.IGNORECASE),
            
            # Личные данные
            'personal_data_ru': re.compile(r'\b(?:фамилия|имя|отчество|дата\s+рождения|место\s+рождения|адрес|прописка|регистрация)\b', re.IGNORECASE),
        }
        return patterns
    ...
    def _compile_code_patterns(self) -> Dict[str, re.Pattern]:
        ...
        patterns = {
            ...
            'sql_commands': re.compile(r'\b(?:SELECT|INSERT|UPDATE|DELETE|DROP|CREATE|ALTER|TRUNCATE|UNION|JOIN|MERGE)\s+', re.IGNORECASE),
            'sql_injection_classic': re.compile(r'\b(?:OR|AND)\s+[\'"]?1[\'"]?\s*=\s*[\'"]?1[\'"]?', re.IGNORECASE),
            'sql_injection_union': re.compile(r'\bUNION\s+(?:ALL\s+)?SELECT\b', re.IGNORECASE),
            'sql_injection_comment': re.compile(r'(?:--|/\*|\*/|#)\s*(?:$|\n)', re.IGNORECASE),
            'sql_injection_stacked': re.compile(r';\s*(?:DROP|DELETE|INSERT|UPDATE|CREATE)\b', re.IGNORECASE),
            'sql_functions': re.compile(r'\b(?:CONCAT|SUBSTRING|ASCII|CHAR|LOAD_FILE|INTO\s+OUTFILE)\s*\(', re.IGNORECASE),
            
            # JavaScript инъекции
            'javascript_tags': re.compile(r'<script[^>]*>.*?</script>', re.IGNORECASE | re.DOTALL),
            'javascript_events': re.compile(r'\bon(?:click|load|error|focus|blur|change|submit|mouseover)\s*=', re.IGNORECASE),
            'javascript_protocols': re.compile(r'\bjavascript\s*:', re.IGNORECASE),
            'javascript_dangerous': re.compile(r'\b(?:eval|setTimeout|setInterval|Function|document\.write|innerHTML|outerHTML)\s*\(', re.IGNORECASE),
            'javascript_dom': re.compile(r'\b(?:document\.cookie|window\.location|document\.location|localStorage|sessionStorage)\b', re.IGNORECASE),
            
            # Command injection (расширенный список)
            'command_dangerous': re.compile(r'\b(?:rm|del|format|fdisk|kill|shutdown|reboot|halt|poweroff|init|systemctl)\s+', re.IGNORECASE),
            'command_network': re.compile(r'\b(?:wget|curl|nc|netcat|telnet|ssh|ftp|sftp|scp|rsync)\s+', re.IGNORECASE),
            'command_system': re.compile(r'\b(?:chmod|chown|sudo|su|passwd|adduser|deluser|crontab|mount|umount)\s+', re.IGNORECASE),
            'command_files': re.compile(r'\b(?:cat|head|tail|grep|find|locate|ls|dir|cp|mv|mkdir|rmdir|touch)\s+', re.IGNORECASE),
            
            # Python code execution
            'python_exec': re.compile(r'\b(?:exec|eval|compile|__import__)\s*\(', re.IGNORECASE),
            'python_dangerous': re.compile(r'\b(?:os\.system|subprocess|shell|popen|spawn)\b', re.IGNORECASE),
            'python_file_ops': re.compile(r'\b(?:open|file|input|raw_input)\s*\(', re.IGNORECASE),
            
            # Shell metacharacters и команды
            'shell_metachar': re.compile(r'[;&|`$()]|\$\([^)]+\)|`[^`]+`|\|\||&&', re.IGNORECASE),
            'shell_redirect': re.compile(r'[<>]{1,2}|>>|2>&1|\d*>&\d*', re.IGNORECASE),
            
            # Другие языки
            'php_dangerous': re.compile(r'\b(?:eval|exec|system|shell_exec|passthru|file_get_contents|include|require)\s*\(', re.IGNORECASE),
            'powershell': re.compile(r'\b(?:Invoke-Expression|IEX|Start-Process|New-Object|Invoke-Command)\b', re.IGNORECASE),
            'cmd_batch': re.compile(r'\b(?:cmd|powershell|wscript|cscript)(?:\.exe)?\s+', re.IGNORECASE),
        }
        return patterns
    ...
    def _compile_model_patterns(self) -> Dict[str, re.Pattern]:
        """Извлечение модели"""
        patterns = {
            'pytorch': re.compile(r'\b(?:model\.state_dict|model\.parameters|torch\.save|model\.load_state_dict)\b', re.IGNORECASE),
            'tensorflow': re.compile(r'\btf\.saved_model\b', re.IGNORECASE),
            'serialization': re.compile(r'\b(?:pickle\.dump|joblib\.dump|numpy\.save)\b', re.IGNORECASE),
            'gradients': re.compile(r'\b(?:grad.*norm|backward\(\)|optimizer\.step)\b', re.IGNORECASE),
            'model_info': re.compile(r'\b(?:model\.modules|model\.named_parameters|model\.summary\(\))\b', re.IGNORECASE),
        }
        return patterns
...
class FastParallelSafetyValidator:
    ...
    async def validate_output_async(self, text: str, user_id: Optional[str] = None) -> SafetyResult:
        """Асинхронная валидация с параллельными проверками"""
        start_time = datetime.now()
        
        # Проверяем лимиты
        if len(text) > self.max_text_length:
            logger.warning(f"⚠️ Текст превышает лимит: {len(text)} > {self.max_text_length}")
            return SafetyResult(
                is_safe=False,
                ...
                details={'text_length': len(text), 'max_length': self.max_text_length, 'user_id': user_id},
                confidence_score=1.0
            )
        
        logger.info("Начало быстрой проверки", extra={
            'user_id': user_id, 
            'text_length': len(text),
            'segment_size': self.segment_size if len(text) > self.segment_size else None
        })
        ...
        # Собираем результаты
        pii_result = results[0] if not isinstance(results[0], Exception) else (False, [], text)
        sensitive_result = results[1] if not isinstance(results[1], Exception) else (False, [], text)
        code_result = results[2] if not isinstance(results[2], Exception) else (False, [])
        model_result = results[3] if not isinstance(results[3], Exception) else (False, [])
        
        # Объединяем результаты
        final_result = self._combine_results(text, pii_result, sensitive_result, code_result, model_result, user_id)
        
        # Время обработки
        processing_time = (datetime.now() - start_time).total_seconds()
        final_result.processing_time = processing_time
        
        logger.info("Быстрая проверка завершена", extra={
            'user_id': user_id,
    ...
    async def _validate_with_segmentation(self, text: str, user_id: Optional[str], start_time: datetime) -> SafetyResult:
        """Валидация больших текстов с сегментацией"""
        logger.info("Сегментация текста", extra={
            'user_id': user_id,
            'text_length': len(text),
        ...
        segments = [text[i:i+self.segment_size] for i in range(0, len(text), self.segment_size)]
        all_violations = []
        all_sanitized_parts = []
        max_risk = 0.0
        ...
        segment_tasks = [process_segment(segment, idx) for idx, segment in enumerate(segments)]
        ...
        high_risk_violations = [v for v in all_violations if any(v.startswith(t) for t in high_risk_types)]
        ...
        logger.info("Сегментация завершена", extra={
            'user_id': user_id,
            'segments_count': len(segments),
            'max_risk': max_risk,
            'violations_count': len(all_violations),
    ...
    def _combine_results(self, text: str, pii_result: Tuple, sensitive_result: Tuple, 
                        code_result: Tuple, model_result: Tuple, user_id: Optional[str]) -> SafetyResult:
    ...
    async def _process_single_segment(self, segment: str) -> Tuple[List[str], str, float]:
        ...
        # Собираем результаты
        pii_result = results[0] if not isinstance(results[0], Exception) else (False, [], segment)
        sensitive_result = results[1] if not isinstance(results[1], Exception) else (False, [], segment)
        code_result = results[2] if not isinstance(results[2], Exception) else (False, [])
        model_result = results[3] if not isinstance(results[3], Exception) else (False, [])
