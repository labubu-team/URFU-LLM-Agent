...
class OfflineOutputSafety:
    ...
    def check_safety(self, text: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                f"🔍 Проверка безопасности для пользователя {user_id}, длина: {len(text)}"
            )
            ...
            logger.info(
                f"{status_emoji} Результат: безопасно={result.is_safe}, "
    ...
    def analyze_text_detailed(
        ...
        analysis = {
            ...
            "security_checks": {
                ...
                "sensitive_data": any(
                    "SENSITIVE_DATA" in v for v in result["violations"]
                ),
                "code_injection": any(
                    "CODE_INJECTION" in v for v in result["violations"]
                ),
                "model_extraction": any(
                    "MODEL_EXTRACTION" in v for v in result["violations"]
                ),
            },
    ...
    def _generate_recommendations(self, result: Dict[str, Any]) -> List[str]:
        ...
        if any("PII_DETECTED" in v for v in result["violations"]):
            recommendations.append(
                "🔒 Обнаружены персональные данные - требуется санитизация"
            )

        if any("CODE_INJECTION" in v for v in result["violations"]):
            recommendations.append(
                "💻 Обнаружен потенциально опасный код - блокировать"
            )
...
def demo_offline_safety():
    ...
    test_cases = [
        ...
        },
        {
            ...
            "text": "Текст содержит email: admin@company.com и API ключ: sk-proj-abcd1234. "
            * 10,
        },
    ]
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        # Быстрая проверка
        is_safe, safe_text = safety.validate_llm_output(
            test_case["text"], f"test_user_{i}"
        )

        status_icon = "✅" if is_safe else "❌"
        print(
            f"{status_icon} Быстрая проверка: {'Безопасно' if is_safe else 'Небезопасно'}"
        )
...
def main():
    ...
    if result["status"] == "success":
        ...
        print(
            f"{status_icon} Результат: {'Безопасно' if basic['is_safe'] else 'Небезопасно'}"
        )
        ...
        if result["sanitized_text"] != text_to_check:
            print("\n🧹 Очищенный текст:")
            print(
                f"{result['sanitized_text'][:300]}{'...' if len(result['sanitized_text']) > 300 else ''}"
            )
