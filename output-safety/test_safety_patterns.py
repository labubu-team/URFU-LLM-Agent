        ...
        for text, should_detect in test_cases:
            with self.subTest(text=text):
                ...
                if should_detect:
                    ...
                    self.assertFalse(any("email" in str(v) for v in violations), 
        ...
        for phone, should_detect in test_cases:
            with self.subTest(phone=phone):
                ...
                if should_detect:
                    ...
                    self.assertTrue(any("phone_ru_mobile" in str(v) for v in violations))
                else:
                    self.assertFalse(any("phone_ru_mobile" in str(v) for v in violations),
...
class TestSensitivePatterns(unittest.TestCase):
    ...
    def test_russian_services(self):
        ...
        for text, should_detect in test_cases:
            with self.subTest(text=text):
                detected, violations, sanitized = self.checker.check_sensitive(text)
                if should_detect:
                    self.assertTrue(detected, f"Должен обнаружить токен российского сервиса в: {text}")
    ...
    def test_medical_data(self):
        ...
        for text in medical_terms_ru + medical_terms_en:
            with self.subTest(text=text):
                detected, violations, sanitized = self.checker.check_sensitive(text)
                self.assertTrue(detected, f"Должен обнаружить медицинские данные в: {text}")
    ...
    def test_sql_injection(self):
        ...
        for text, should_detect in test_cases:
            with self.subTest(text=text):
                ...
                if should_detect:
                    self.assertTrue(detected, f"Должен обнаружить SQL инъекцию в: {text}")
    ...
    def test_javascript_injection(self):
        ...
        for text, should_detect in test_cases:
            with self.subTest(text=text):
                detected, violations = self.checker.check_code(text)
                if should_detect:
                    self.assertTrue(detected, f"Должен обнаружить JavaScript инъекцию в: {text}")
    ...
    def test_command_injection(self):
        ...
        for text, should_detect in test_cases:
            with self.subTest(text=text):
                detected, violations = self.checker.check_code(text)
                if should_detect:
                    self.assertTrue(detected, f"Должен обнаружить command injection в: {text}")
    ...
    def test_python_code_execution(self):
        ...
        for text, should_detect in test_cases:
            with self.subTest(text=text):
                detected, violations = self.checker.check_code(text)
                if should_detect:
                    self.assertTrue(detected, f"Должен обнаружить Python code execution в: {text}")
    ...
    def test_pytorch_patterns(self):
        ...
        for text, should_detect in test_cases:
            with self.subTest(text=text):
                ...
                if should_detect:
                    self.assertTrue(detected, f"Должен обнаружить PyTorch паттерн в: {text}")
