...
def run_unit_tests():
    ...
    from test_safety_patterns import (
        TestPIIPatterns, TestSensitivePatterns, 
        TestCodeInjectionPatterns, TestModelExtractionPatterns,
        TestIntegratedValidator, TestPerformance
    )
...
def demo_safety_validation():
    ...
    test_cases = [
        ...
        },
        {
            ...
            "text": ("Обычный текст " * 100) + " с опасностью: SELECT password FROM users WHERE admin = 'true'",
            "expected_safe": False
        }
    ]
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        print(f"{safety_icon} Безопасность: {'Безопасно' if result.is_safe else 'Небезопасно'}")
        ...
        if result.violations:
            print(f"[НАРУШЕНИЯ] Типы нарушений:")
        ...
        if result.sanitized_text != test_case['text']:
            print(f"[ОЧИСТКА] Очищенный текст: {result.sanitized_text[:100]}{'...' if len(result.sanitized_text) > 100 else ''}")
...
async def demo_async_performance():
    ...
    for i, result in enumerate(parallel_results):
        print(f"   Текст {i+1}: {len(test_texts[i])} символов → {result.processing_time:.3f}s")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        mode_map = {'1': 'tests', '2': 'demo', '3': 'async', '4': 'benchmark', '5': 'all'}
