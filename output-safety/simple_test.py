...
def test_basic_functionality():
    ...
    assert result.is_safe == False, "Текст с email должен быть небезопасным"
    print("  [OK] Тест пройден")
    ...
    print(f"  Риск: {result.risk_score:.3f}")
    print(f"  Нарушений: {len(result.violations)}")
    assert result.is_safe == False, "SQL инъекция должна быть небезопасной"
    ...
    print(f"  Риск: {result.risk_score:.3f}")
    print(f"  Нарушений: {len(result.violations)}")
    assert result.is_safe == False, "Текст с телефоном должен быть небезопасным"
    print("  [OK] Тест пройден")
...
def test_performance():
    ...
    print(f"Огромный текст ({len(huge_text)} символов): {'Заблокирован' if not result.is_safe else 'Пропущен'}")
