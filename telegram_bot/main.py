...
_DEFAULT = {"name","msg","args","levelname","levelno","pathname","filename","module",
            "exc_info","exc_text","stack_info","lineno","funcName","created","msecs",
            "relativeCreated","thread","threadName","processName","process"}

class YCJsonFormatter(logging.Formatter):
    def format(self, rec: logging.LogRecord) -> str:
        sev = {"WARNING":"WARN","CRITICAL":"FATAL"}.get(rec.levelname, rec.levelname).upper()
        payload: Dict[str, Any] = {
            "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S", time.gmtime(rec.created)) + f".{int(rec.msecs):03d}Z",
            "severity": sev, "logger": rec.name, "event": getattr(rec, "event", rec.funcName or "log"),
            "message": rec.getMessage(),
        }
        for k, v in rec.__dict__.items():
            if k not in _DEFAULT and not k.startswith("_"):
                payload[k] = v
        if rec.exc_info: payload["exception"] = self.formatException(rec.exc_info)
...
def _llm_url() -> str:
    return f"{LLM_AGENT_URL}/v1/completion" if LLM_AGENT_URL else f"http://{LLM_AGENT_HOST}:{LLM_AGENT_PORT}/v1/completion"

class YandexGPTBot:
    def ask_gpt(self, q: str) -> str:
        t0 = time.perf_counter(); url = _llm_url()
        try:
            r = requests.post(url, headers={"Content-Type":"application/json"},
                              json={"messages":[{"role":"user","text":q}]}, timeout=15)
            dt = round((time.perf_counter()-t0)*1000,1)
            jinfo("LLM request done", event="llm_request", url=url, status=r.status_code, duration_ms=dt, bytes=len(r.content))
            r.raise_for_status(); j = r.json()
            return j["result"]["alternatives"][0]["message"]["text"]
        except Exception as e:
            dt = round((time.perf_counter()-t0)*1000,1)
            jerror("LLM request failed", event="llm_error", url=url, duration_ms=dt, error=str(e)); raise
llm = YandexGPTBot()

async def start_cmd(u: Update, c: ContextTypes.DEFAULT_TYPE):
    usr = u.effective_user
    jinfo("Start command", event="tg_command", command="start", user_id=getattr(usr,"id",None), username=getattr(usr,"username",None))
...
async def msg_handler(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    jinfo("Incoming message", event="tg_message", chat_id=getattr(u.effective_chat,"id",None),
          user_id=getattr(usr,"id",None), username=getattr(usr,"username",None),
          message_id=getattr(msg,"message_id",None), text_len=len(text))
    if not text:
        await msg.reply_text("Пожалуйста, введите вопрос."); return
    try:
...
async def healthz(_: web.Request):
    return web.json_response({"status": "alive", "uptime_seconds": round(time.time()-STARTED_AT, 3)})
...
async def readyz(_: web.Request):
    ...
    return web.json_response({"status": "ok" if ok else "degraded", "dependencies": deps}, status=200 if ok else 503)

async def tg_webhook(req: web.Request):
    if req.method != "POST": return web.Response(status=405)
...
async def amain():
    if not TELEGRAM_TOKEN: raise RuntimeError("TELEGRAM_TOKEN must be set")
    global ptb
