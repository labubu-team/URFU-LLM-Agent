...
class YCJsonFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord) -> str:
        sev = {"WARNING":"WARN","CRITICAL":"FATAL"}.get(record.levelname, record.levelname).upper()
...
class YandexGPTBot:
    def ask_gpt(self, question: str) -> str:
        ...
        try:
            ...
            jinfo("LLM request done", event="llm_request", url=url, status=resp.status_code, duration_ms=dt, bytes=len(resp.content))
            resp.raise_for_status()
            j = resp.json()
            return j["result"]["alternatives"][0]["message"]["text"]
        except Exception as e:
            dt = round((time.perf_counter() - t0) * 1000, 1)
            jerror("LLM request failed", event="llm_error", url=url, duration_ms=dt, error=str(e))
            raise
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    jinfo("Start command", event="tg_command", command="start", user_id=getattr(u, "id", None), username=getattr(u, "username", None))
...
async def msg_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    jinfo("Incoming message", event="tg_message", chat_id=getattr(update.effective_chat, "id", None),
...
async def _polling_loop():
    ...
    while True:
        try:
            updates = await ptb_app.bot.get_updates(offset=offset, timeout=50, request_timeout=60)
            for upd in updates:
